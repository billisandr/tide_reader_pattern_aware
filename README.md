...
### Step 1: Run Pattern-Aware Detection
...
```yaml
# In config.yaml - System automatically uses pattern-aware detection
detection:
  pattern_aware:
...
python src/interactive_template_extractor.py
```
...
The pattern-aware system uses the following dependencies:
...
# Verify installation
python -c "from src.pattern_water_detector import PatternWaterDetector; print('Pattern-aware system ready')"
```
...
detection:
  pattern_aware:
    ...
    # Waterline verification system
    waterline_verification:
      enabled: true                     # Enable waterline verification
...
```python
from src.detection_methods.e_pattern_detector import EPatternDetector
...
python src/extract_templates.py --scale-type stadia_rod
```
...
2. **Waterline Analysis** - Post-processing gradient validation
...
├── hybrid_waterline_analyzer.py       # Advanced waterline analysis system
...
│   ├── analyze_scale_photo.py        # Interactive scale analysis
│   └── quick_scale_analysis.py       # Quick calibration utility
...
- **HybridWaterlineAnalyzer**: Advanced waterline analysis with combined metric detection
- **IntegratedPatternDetector**: Orchestrates multiple detection methods with intelligent prioritization
